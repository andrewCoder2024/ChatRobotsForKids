...
class Chat(object):
    ...
    def converse(self, input_sentence, session_id="general"):
        """
        ...
        response = self._say(session, input_sentence)
        response = response.replace("ChatBot", "Qilin")
        return response


if __name__ == "__main__":
    chatbot = Chat(language='en')
    while True:
        text = input(">> ")
        if text == "quit":
            break
        print(chatbot.converse(text))